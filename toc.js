(function(){
  ...
  function init(){
    ...
    var headingOffsets = [];
    function cacheOffsets(){ headingOffsets = headings.map(function(h){ return h.offsetTop; }); }
    cacheOffsets();
    var resizeTimer;
    window.addEventListener('resize', function(){ clearTimeout(resizeTimer); resizeTimer = setTimeout(cacheOffsets, 300); });
    setTimeout(cacheOffsets, 1500); setTimeout(cacheOffsets, 4000);
